class RailWay
  ...
  def all_stations_list
    ...
    if self.stations.empty?
      ...
      self.stations.each_with_index do |station, s_index| 
        ...
        if station.trains.empty?
          ...
          station.train_list do |t_index, train_info| 
            ...
            if train_info[:obj].wagons.size == 0
              puts "Train has no attached wagons."
            else
              train_info[:obj].wagons_list { |w_index, wagon_info| puts "#{s_index + 1}.#{t_index + 1}.#{w_index + 1}. wagon number: #{wagon_info[:number]} | type: #{wagon_info[:type]} | vacant: #{wagon_info[:vacant]} | reserved: #{wagon_info[:reserved]}" }
            end            
          end
        end
      end
      ...
      if station.trains.empty?
        ...
        station.train_list do |t_index, train_info| 
          ...
          if train_info[:obj].wagons.size == 0
            ...
            train_info[:obj].wagons_list { |w_index, wagon_info| puts "#{t_index + 1}.#{w_index + 1}. wagon number: #{wagon_info[:number]} | type: #{wagon_info[:type]} | vacant: #{wagon_info[:vacant]} | reserved: #{wagon_info[:reserved]}" }
          end            
        end
      end
    end
  ...
  def reserve_seat(passenger_wagon)
    #puts "Enter seats amount to reserve:"
    #amount = gets.chomp.to_i
    if passenger_wagon.vacant_seats > 0
  ...
  def select_wagon(train)
    if train.wagons.empty?
      ...
      show_wagons_by_train(train)
      puts "Choose wagon:"
      return train.wagons[gets.chomp.to_i - 1]
    end
  end
  ...
  def show_wagons_by_train(train)
    if train.wagons.size == 0
      ...
      train.wagons_list { |index, wagon_info| puts "#{index + 1}. number: #{wagon_info[:number]} | type: #{wagon_info[:type]} | vacant: #{wagon_info[:vacant]} | reserved: #{wagon_info[:reserved]}" }
    end
  end
